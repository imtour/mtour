...
/**
 * 设置访问密码
 * <a href="" class="return-home" title="点击回到网站首页">- 内部使用，请见谅！ - </a>
 * @param $password  访问密码
...
function MkEncrypt($password, $pageid = 'default') {
    ...
    if($postpwd == $password) {         // 提交的密码正确   time()+3600*24*3      setcookie('mk_encrypt_' . $pageid, $md5pw, time() + 3600000, '/');  
        //setcookie('mk_encrypt_' . $pageid, $md5pw, time() + 3600*24*3, '/');
        setcookie('mk_encrypt_' . $pageid, $md5pw, time() + 36000, '/');
        return;
    }
...
<body>
    <div class="main">
        ...
        <h1 class="" class="return-home" title="">内部使用，请见谅！</h1> 
    </div>
